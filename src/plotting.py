...
def plot_breakeven(results: pd.DataFrame, name="05_breakeven_price"):
    """
    Three-zone view of the two electricity-price thresholds.

    Each bar runs to the current Ofgem average and splits into:
      blue   — below the profitability threshold: the investment pays back
      amber  — cheaper to run than gas, but capital not recovered
      red    — more expensive to run than gas (where we are today)

    The point of the chart is that clearing the red band only gets you into
    amber; profitability needs the price to fall into the blue.
    """
    fig, ax = _canvas((12, 6.8))

    dwellings = results["Dwelling type"].tolist()
    profitable = results["Profitable price (p/kWh)"].tolist()
    parity = results["Running parity price (p/kWh)"].tolist()

    avg = config.OFGEM_ANNUAL_AVG_P
    q = list(config.OFGEM_QUARTERLY_P.values())
    x = np.arange(len(dwellings))

    # Quarterly range as a single band rather than four cramped lines
    ax.axhspan(min(q), max(q), color="#cbd5e1", alpha=0.30, zorder=1)
    ax.text(len(dwellings) - 0.30, (min(q) + max(q)) / 2,
            f"Ofgem 2025\nquarterly range\n{min(q):.2f}–{max(q):.2f}p",
            fontsize=9, color=INK_SOFT, va="center", ha="left", linespacing=1.4)

    seg_parity = [p - v for p, v in zip(parity, profitable)]
    seg_loss = [avg - p for p in parity]

    ax.bar(x, profitable, 0.5, color=BLUE, zorder=3,
           label="Profitable — investment recovers its capital")
    ax.bar(x, seg_parity, 0.5, bottom=profitable, color="#e8a33d", alpha=0.9,
           zorder=3, label="Cheaper to run, but capital not recovered")
    ax.bar(x, seg_loss, 0.5, bottom=parity, color=RED, alpha=0.85,
           zorder=3, label="More expensive to run than a gas boiler")

    for xi, pf, pa in zip(x, profitable, parity):
        ax.text(xi, pf / 2, f"{pf:.2f}p", ha="center", va="center",
                fontsize=11, color="white", fontweight="600")
        ax.text(xi, pf + (pa - pf) / 2, f"{pa:.2f}p", ha="center", va="center",
                fontsize=9.5, color="white", fontweight="600")
        ax.text(xi, pa + (avg - pa) / 2, f"−{avg - pf:.2f}p\nto profitability",
                ha="center", va="center", fontsize=8.5, color="white",
                fontweight="600", linespacing=1.3)

    ax.axhline(avg, color=ORANGE, lw=2.0, zorder=5,
               label=f"Ofgem 2025 average ({avg:.2f} p/kWh)")

    ax.set_xticks(x)
    ax.set_xticklabels(dwellings, color=INK, fontsize=10.5)
    ax.set_ylabel("Electricity price (p/kWh)", color=INK_FAINT, fontsize=10)
    ax.set_ylim(0, avg * 1.22)
    ...
    ax.legend(fontsize=9, loc="upper center", bbox_to_anchor=(0.5, -0.09),
              ncol=2, facecolor=BG_PANEL, edgecolor=GRID, labelcolor=INK_SOFT)

    _title(ax, "Electricity price required for retrofit to become profitable",
           f"{config.APPRAISAL_YEARS}-year NPV at "
           f"{config.DISCOUNT_RATE*100:.1f}%  ·  net of "
           f"£{config.BUS_GRANT_GBP:,} BUS grant  ·  gas "
           f"{config.GAS_PRICE_P_PER_KWH:.2f} p/kWh at "
