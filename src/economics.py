...
def running_parity_price_p(scop_total: float) -> float:
    """
    ...
    This is a running-cost measure only. It contains no capital term, no
    appraisal horizon and no discount rate, so reaching it means the annual
    operating penalty disappears — not that the investment pays back.
    """
    gas_heat_cost_p = config.GAS_PRICE_P_PER_KWH / config.BOILER_EFFICIENCY
    return gas_heat_cost_p * scop_total


def profitable_price_p(
    gas_cost_gbp: float,
    ...
    """
    Electricity price at which NPV reaches zero over the appraisal period, p/kWh.

    This is the threshold that matters to a household: the price at which the
    switch not only stops costing more to run but also recovers its capital.

    Setting NPV = 0:
        -C_net + S * AF = 0        ->  S = C_net / AF
        S = gas_cost - (demand / SCOP) * P / 100
        P = (gas_cost - C_net / AF) / (demand / SCOP) * 100

    It sits well below running parity, because the annual saving must be
    positive enough to amortise the capital rather than merely reach zero.
    """
    af = annuity_factor(rate, years)
    required_saving = net_capital_gbp / af
    allowable_ashp_cost = gas_cost_gbp - required_saving
    elec_kWh = annual_demand_kWh / scop_total
    return allowable_ashp_cost / elec_kWh * 100
...
def appraise(df: pd.DataFrame, dwelling: str) -> dict:
    ...
    parity = running_parity_price_p(cop["scop_total"])
    profitable = profitable_price_p(
        gas_cost, total_demand, cop["scop_total"], net_capital
    )
    ...
    result = {
        ...
        # Unrounded, for calculations that amplify the value (e.g. grant
        # requirement multiplies by the annuity factor, so rounding here
        # would break the grant/NPV identity by several pounds).
        "_saving_exact": saving,
        ...
        # Threshold 1 — running-cost parity (no capital)
        "Running parity price (p/kWh)": round(parity, 2),
        # Threshold 2 — NPV = 0 (the household-relevant threshold)
        "Profitable price (p/kWh)": round(profitable, 2),
        "Reduction to profitability (p/kWh)": round(
            config.OFGEM_ANNUAL_AVG_P - profitable, 2
        ),
        "Reduction to profitability (%)": round(
            100 * (config.OFGEM_ANNUAL_AVG_P - profitable)
    ...
    # Exact 15-year NPV, for the identity check against the grant requirement
    result["_npv_exact"] = net_present_value(
        net_capital, saving, config.APPRAISAL_YEARS
    )
